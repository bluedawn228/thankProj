...
.bgArticle {
	...
	width: 100%;
	height: 100%;
	background: url('/images/background/blue-259458.jpg') center center/cover;
	opacity: 0.1;
}
...
function deleteDiary(diaryId){
	var con_msg=confirm("정말 삭제하시려구요?");
	if (con_msg == true) {
		location.href='/deleteDiary?diaryId='+diaryId;
	}
	
}

function cancleShare(diaryId){
	var con_msg=confirm("비공개로 바꾸시겠어요?");
	if (con_msg == true) {
		location.href='/cancleDiaryShare?diaryId='+diaryId;
	}
}

function openDiary(diaryId){
	var con_msg=confirm("공개로 바꾸시겠어요?");
	if (con_msg == true) {
		location.href='/openDiary?diaryId='+diaryId;
	}	
}
</script>
				...
				<div id="contents" class="container">
					<div class="cont-mid safety-why">
						...
						<article class="board-view entireArticle">
							...
							<h1 class="board-view-title">${diary.author}님의 ${aCnt}번째
								감성일기</h1>
							<div class="tag-sns-wrap clearfix">
								<div class="tag-time">
									<span class="number-tag"><i class="xi-clock-o"></i> <fmt:formatDate
											value="${diary.regdate}" pattern="yyyy년 MM월 dd일 EE요일" /><br></span>
								</div>
								<div class="sns-link">
									<span>조회수 ${diary.viewCnt}</span> <span>댓글 수
										${diary.commCnt}</span>
								</div>
							</div>
							<div class="view-content-wrap">
								<div class="view-content">
									...
									<div style="text-align: center;">
										<h4>오늘의 명언</h4>
										<span class="phrsText"> ${phrs.phrsText}</span> <br> <span
											class="phrsAuthor">- ${phrs.author} -</span> <br>
									</div>
							...
							<ul class="tag-file-list">
								<li class="list-tag"><span>태그</span>
									<ul class="tag-list">
								...
								<li class="list-tag"><span>첨부파일</span>
									<div class="file-list">파일 상세</div></li>
							</ul>
							<div class="tag-sns-wrap clearfix"></div>

							<article class="board-view text-center">
								<div class="btn-group">
									<button type="button" id="recommendation-btn"
										class="btn rcmdbtn" data-bs-toggle="modal"
										data-bs-target="#exampleModal"
										onclick="javascript:diaryRcmd(${diary.diaryId});">
									...
									<button type="button" class="btn rcmdbtn" id="rcmdCNT">
										<fmt:formatNumber value="${diary.rcmdCnt}"
											pattern="###,###,###,###" />
									</button>
								</div>
							</article>
						...
						<article>
							...
							<form name="comments-form" id="comments-form"
								action="./addDiaryComm" method="post">
								<div class="card">
									...
									<div style="width: 100%; text-align: center;">
										<div
											...
											<div style="display: inline-block;">
												...
												<button class="btn btn-primary" type="button" id="comments-btn" style="height: 100px; float: left;" onclick="addComment(${diary.diaryId})">댓글 달기</button>
						...
						<div class="btns txt-right">
							<button type="button" class="btn btn-style01" id="btnList" onclick="javascript:history.back()">
							...
							<button type="button" class="btn btn-style01" id="btnList" onclick="deleteDiary('${diary.diaryId}')">
								<i class="xi-bars"></i>삭제
							</button>
							<c:choose>
								<c:when test="${post.opened}==1">
								<button type="button" class="btn btn-style01" id="btnList" onclick="cancleShare('${diary.diaryId}')">
									<i class="xi-bars"></i>공유취소
								</button>
								</c:when>
								<c:otherwise>
									<button type="button" class="btn btn-style01" id="btnList" onclick="openDiary('${diary.diaryId}')">
										<i class="xi-bars"></i>공개하기
									</button>
								</c:otherwise>
							</c:choose>	
						</div>
